...
export class HomeSelectionComponent {
    ...
    public navigateToSchemes(tabType: TabType): void {
        const newTab = new Tab('newTab', tabType)
        this.schemesService.addTab(newTab)
        const tabsSize = this.schemesService.getTabs().length
    ...
    private navigateToTab(tabsSize) {
        ...
        this.router
            ...
            .then((value) =>
                Logger.i(
                    this,
                    'New tab created from the [HomeComponent], result is [' +
                        value +
                        ']'
                )
            )
            .catch((reason) =>
                Logger.e(
                    this,
                    'Error occurred while navigating to the new tab on [HomeComponent], reason:',
                    reason
                )
            )
    }
