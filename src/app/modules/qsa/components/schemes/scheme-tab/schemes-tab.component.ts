...
export class SchemesTabComponent implements OnInit, OnDestroy {
    ...
    public ngOnInit(): void {
        this.subscription = this.schemesService.getOnTabsChangedListener().subscribe((tabs: Tab[]) => this.tabs = tabs);
    ...
    public onNewTab(): void {
        let newTab = new Tab('newTab');
        this.schemesService.addTab(newTab);
        this.schemesService.getOnChangedListener().next(this.tabs.length - 1);
        this.router.navigate([(this.tabs.length - 1)], {relativeTo: this.route})
            .then(value => Logger.i(this, 'New tab created from the [SchemesTabComponent], result is [' + value + ']'))
            .catch(reason => Logger.e(this, 'Error occurred while navigating to the new tab on [SchemesTabComponent], reason:', reason));
    }
}
