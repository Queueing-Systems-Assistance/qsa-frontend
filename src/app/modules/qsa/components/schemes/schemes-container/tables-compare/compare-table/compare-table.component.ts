...
export class CompareTableComponent {
    ...
    public getReadableValue(
        ...
        const value = this.getSystemFeatureValueToDisplay(
            tableNumber,
            systemFeature
        )
        let result = ''
    ...
    public getSystemFeatureValueToDisplay(
        tableNumber: number,
        systemFeature: SystemFeature
    ): number {
        ...
        return tableView.systemOutputs
            .filter(
                (systemFeatureValue) =>
                    systemFeatureValue.id === systemFeature.id
            )
    ...
    public getSystemFeatures(): SystemFeatureValue[] {
        ...
        result = result.concat(
            this.getSystemFeaturesDistinct(
                this.tableViewCompare.tableView1.systemOutputs,
                result
            )
        )
        result = result.concat(
            this.getSystemFeaturesDistinct(
                this.tableViewCompare.tableView2.systemOutputs,
                result
            )
        )
        return result
    ...
    public getSystemFeaturesDistinct(
        ...
        return result.concat(
            systemFeatureValues.filter(
                (systemFeature) =>
                    !this.isSystemFeaturesAlreadyAdded(systemFeature, result)
            )
        )
    }
