...
export class CompareTableSelectionComponent implements OnInit {
    ...
    @Output() onTableCompareCreated: EventEmitter<TableViewCompare> = new EventEmitter<TableViewCompare>();
    ...
    public ngOnInit(): void {
        this.getTableViewTabIndexes()
            .forEach(value => this.formGroup.addControl('' + value, new FormControl(false)));
    }

    public isTwoTablesAreSelected(): boolean {
        return this.getTableViewTabIndexes()
            .filter(tabIndex => this.formGroup.value['' + tabIndex]).length >= 2;
    }

    public setSelectable(): void {
        let isTwoTablesAreSelected = this.isTwoTablesAreSelected();
        this.getTableViewTabIndexes()
            .forEach(tabIndex => {
                let currentSystemSelected = this.formGroup.value['' + tabIndex];
    ...
    public processCompareTableScheme(): void {
        ...
        let tableViewCompare = this.createTableViewCompare();
        this.compareTableViewService.addTableViewCompare(tableViewCompare, this.currentTab);
        this.schemesService.getSelectedTab(this.currentTab).type = TabType.COMPARE_TABLE;
    ...
    private createTableViewCompare(): TableViewCompare {
        let tableViews = CompareTableSelectionComponent.copyObject(this.getSelectedTableView());
        let tabNames = CompareTableSelectionComponent.copyObject(this.getTableViewSelectedTabNames());
        Logger.i(this, 'Selected tables', tableViews);
        Logger.i(this, 'Selected tabs', tabNames);
        return new TableViewCompare(tableViews[0], tableViews[1], tabNames[0], tabNames[1]);
    }
