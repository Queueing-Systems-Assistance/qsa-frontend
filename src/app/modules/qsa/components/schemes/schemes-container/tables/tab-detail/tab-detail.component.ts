...
export class TabDetailComponent implements OnInit {
    ...
    public calculateSystemFeatures(): void {
        ...
        this.backendService
            .getTable(this.getSystemInputForm().value, this.getSystemViewId())
            .subscribe(
                (value) => {
                    value.systemView = this.getSystemView()
                    return this.tablesService.addTableView(
                        this.currentTab,
                        value
                    )
                },
    ...
    public getSystemInputLayouts(id: string): void {
        this.tablesService.addSystemView(
            this.currentTab,
            this.systemViewService.getSystemViewById(id)
        )
    ...
    private updateSystemViewForms(): void {
        if (!this.getSystemViewsForm()) {
            const newSystemViewsForm = new FormGroup({
                systemViewSelected: new FormControl(null),
            })
            this.tablesService.addSystemViewsForm(
                this.currentTab,
                newSystemViewsForm
            )
        }
    ...
    private updateSystemInputForms(
        systemViewInputs: Array<SystemFeature>
    ): void {
        if (
            (!this.getSystemInputForm() && systemViewInputs) ||
            systemViewInputs
        ) {
            ...
            this.tablesService.addSystemInputsForm(
                this.currentTab,
                newInputFormGroup
            )
            this.tablesService.addSystemViewInputs(
                this.currentTab,
                systemViewInputs
            )
        }
