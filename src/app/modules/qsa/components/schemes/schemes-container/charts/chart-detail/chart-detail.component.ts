...
export class ChartDetailComponent implements OnInit {
    ...
    public isXAxisSelected(): boolean {
        let featureId = this.chartsService.getSystemInputsForm(this.currentTab).value.xAxis.featureId;
    ...
    private updateSystemViewsForms(): void {
        let savedSystemViewsForm = this.getSystemViewsForm();
        Logger.i(this, 'Saved system views form', savedSystemViewsForm);
        if (!savedSystemViewsForm) {
            this.chartsService.addSystemViewsForm(this.currentTab, new FormGroup({systemViewSelected: new FormControl()}));
        }
    }
    ...
    private createXAxisForms(): void {
        ...
        let systemInputsForm = new FormGroup({
            xAxis: new FormGroup({
    ...
    private addDynamicInputForms(dynamicInputForms: FormGroup): void {
        ...
        this.getSystemViewInputs()
            .forEach(systemInput => this.getSystemInputsForm()[systemInput.id] = 0);
    }
    ...
    private createDynamicInputForms(): FormGroup {
        let dynamicForms = new FormGroup({});
        this.getSystemViewInputs()
            .forEach(systemInput => dynamicForms.addControl(systemInput.id, new FormControl(0)));
        return dynamicForms;
    }
