...
export class ChartDetailComponent implements OnInit {
    ...
    public selectSystem(systemViewId: string): void {
        ...
        this.chartsService.addSystemViewsForm(
            this.currentTab,
            this.getSystemViewsForm()
        )
        this.chartsService.addSystemView(
            this.currentTab,
            this.systemViewService.getSystemViewById(systemViewId)
        )
    ...
    public isXAxisSelected(): boolean {
        const featureId = this.chartsService.getSystemInputsForm(
            this.currentTab
        ).value.xAxis.featureId
        return (
            featureId &&
            this.chartsService.getSystemInputsForm(this.currentTab).value[
                this.getSystemViewId()
            ]
        )
    }
    ...
    public getRequiredFixSystemInputs(): SystemFeature[] {
        return this.createFixSystemInputs().filter(
            (systemViewInput) => systemViewInput.required
        )
    }
    ...
    public getXAxisName(): string {
        ...
        if (this.getSystemViewInputs()) {
            xAxisName = this.getSystemViewInputs()
                .filter(
                    (systemViewInput) =>
                        this.getFeatureId() === systemViewInput.id
                )
    ...
    private updateSystemInputsForms(newInputs: boolean): void {
        if (
            (!this.getSystemInputsForm() && this.getSystemViewInputs()) ||
            newInputs
        ) {
    ...
    private createXAxisForms(): void {
        ...
        this.chartsService.addSystemInputsForm(
            this.currentTab,
            systemInputsForm
        )
    }
    ...
    private addDynamicInputForms(dynamicInputForms: FormGroup): void {
        if (!this.isXAxisSelected()) {
            ...
            this.getSystemInputsForm().addControl(
                this.getSystemViewId(),
                dynamicInputForms
            )
        }
        this.getSystemViewInputs().forEach(
            (systemInput) => (this.getSystemInputsForm()[systemInput.id] = 0)
        )
    }

    private createFixSystemInputs(): Array<SystemFeature> {
        return this.getSystemViewInputs().filter(
            (systemInput) => this.getFeatureId() !== systemInput.id
        )
    }

    private createDynamicInputForms(): FormGroup {
        const dynamicForms = new FormGroup({})
        this.getSystemViewInputs().forEach((systemInput) =>
            dynamicForms.addControl(systemInput.id, new FormControl(0))
        )
        return dynamicForms
    }
    ...
    private getFeatureId() {
        if (
            this.getSystemInputsForm() === undefined ||
            this.getSystemInputsForm().value.xAxis === undefined
        ) {
            return null
        }
